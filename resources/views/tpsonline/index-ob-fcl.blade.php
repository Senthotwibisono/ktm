...
<script>
    ...
    {
        var ids = jQuery("#tpsobFclGrid").jqGrid('getDataIDs'),
            edt = '',
            del = ''; 
        for(var i=0;i < ids.length;i++){ 
            var cl = ids[i];
            
            edt = '<a href="{{ route("tps-obFcl-edit",'') }}/'+cl+'"><i class="fa fa-pencil"></i> Details</a> ';
//            del = '<a href="{{ route("fcl-register-delete",'') }}/'+cl+'" onclick="if (confirm(\'Are You Sure ?\')){return true; }else{return false; };"><i class="fa fa-close"></i></a>';
            jQuery("#tpsobFclGrid").jqGrid('setRowData',ids[i],{action:edt}); 
        } 
    }
...
<div class="box">
    ...
    <div class="box-body table-responsive">
        ...
        {{
            GridRender::setGridId("tpsobFclGrid")
            ->enableFilterToolbar()
            ->setGridOption('mtype', 'POST')
            ->setGridOption('url', URL::to('/tpsonline/penerimaan/ob-fcl/grid-data?jenis=F&_token='.csrf_token()))
...
//            ->addColumn(array('label'=>'Action','index'=>'action', 'width'=>80, 'search'=>false, 'sortable'=>false, 'align'=>'center'))
            ...
            ->addColumn(array('label'=>'Janis Cont.','index'=>'JNS_CONT','width'=>160))
...
<script type="text/javascript">
    ...
    $('#searchByDateBtn').on("click", function(){
        ...
        jQuery("#tpsobFclGrid").jqGrid('setGridParam',{url:"{{URL::to('/tpsonline/penerimaan/ob-fcl/grid-data')}}?jenis=F&startdate="+startdate+"&enddate="+enddate+"&by="+by}).trigger("reloadGrid");
        return false;
    });
</script>
