...
<div class="box">
    <div class="box-header with-border">
        <h3 class="box-title">COARI CODECO Terkirim</h3>
        <div class="box-tools">
            <a href="{{ route('tps-terkirim-get') }}" type="button" class="btn btn-block btn-info btn-sm"><i class="fa fa-plus"></i> Get Data</a>
        </div>
    </div>
    <div class="box-body table-responsive">
        <div class="row" style="margin-bottom: 30px;margin-right: 0;">
            ...
            {{
                GridRender::setGridId("tpsTerkirimGrid")
                ->enableFilterToolbar()
                ->setGridOption('url', URL::to('/tpsonline/report/grid-data?type=terkirim'))
//                ->setGridOption('editurl',URL::to('/tpsonline/report/crud'))
                ->setGridOption('rowNum', 20)
                ->setGridOption('shrinkToFit', true)
                ->setGridOption('sortname','tps_getdatakirim_pk')
                ...
                ->addColumn(array('key'=>true,'index'=>'tps_getdatakirim_pk','hidden'=>true))
...
//    $('select').select2();
    ...
    $('#searchByDateBtn').on("click", function(){
        ...
        jQuery("#tpsTerkirimGrid").jqGrid('setGridParam',{url:"{{URL::to('/tpsonline/report/grid-data')}}?type=terkirim&startdate="+startdate+"&enddate="+enddate+"&by="+by}).trigger("reloadGrid");
        return false;
    });
</script>
